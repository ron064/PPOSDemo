...
static void playersLayer_update_callback(Layer *me, GContext* ctx) {
  ...
  graphics_context_set_compositing_mode(ctx, foreColor == GColorWhite ? GCompOpAssignInverted : GCompOpAssign);
...
void redraw_toe() {
  text_layer_set_text_color(timeLayer, foreColor);
...
void load_toe() {
  ...
  layer_add_child(rootLayer, boardLayer); // Add the child to the app's base window
  ...
  layer_add_child(rootLayer, playersLayer);
  ...
  layer_add_child(rootLayer, text_layer_get_layer(timeLayer));
...
void info_toe(void* disp_info)
{
	display_info * info = (display_info *) disp_info;
	snprintf(info->app_name, sizeof(info->app_name),"tic tock toe");
